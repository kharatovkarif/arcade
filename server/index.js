...
// Ad 5 — Monetag Rewarded Interstitial, client-triggered with auth + daily limit
app.post('/api/ads/watch5', auth, async (req, res) => {
  const todayStart = mskDate() + 'T00:00:00+03:00';
  ...
  const current = count || 0;
  if (current >= limit) return res.json({ ok: false, error: 'daily_limit', daily_count: limit });
  if (await adCooldownActive(req.user.tg_id, 'ad5'))
    return res.json({ ok: false, error: 'cooldown', daily_count: current });
...
// Ad 7 — Telega.io Rewarded, client-triggered with auth + daily limit
app.post('/api/ads/watch7', auth, async (req, res) => {
  const todayStart = mskDate() + 'T00:00:00+03:00';
  const limit = isPro(req.user) ? 15 : 10;
  ...
  const current = count || 0;
  if (current >= limit) return res.json({ ok: false, error: 'daily_limit', daily_count: limit });
  if (await adCooldownActive(req.user.tg_id, 'ad7'))
    return res.json({ ok: false, error: 'cooldown', daily_count: current });
